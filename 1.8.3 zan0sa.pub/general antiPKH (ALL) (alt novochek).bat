...
--filter-tcp=443 --hostlist-exclude="%LISTS%zapret-hosts-exclude.txt" --hostlist="%LISTS%list-zapret.txt" --hostlist-auto="%LISTS%zapret-hosts-auto.txt" --hostlist="%LISTS%list-general.txt" --dpi-desync=multisplit --dpi-desync-repeats=2 --dpi-desync-split-seqovl=681 --dpi-desync-split-pos=1 --dpi-desync-split-seqovl-pattern="%BIN%tls_clienthello_www_google_com.bin" --new ^
